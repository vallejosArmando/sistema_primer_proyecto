...
<body >
    <form action="conexion14.php" id="form1" name="form1" method="POST" enctype="multipart/form-data">
        <fieldset>
            
            <legend class="leg">Datos reales del reclamo:</legend>

            <div id="div"><label for="id_reclamo">Reclamos</label>
            <select name="id_reclamo" id="id_reclamo"  >
                <option value="">Reclamo de:</option>
                <?php 
              require '../conexion.php';

                $v=mysqli_query($con,"SELECT * FROM reclamos");
while ($reclamos=mysqli_fetch_row($v)){ ?>
    
                <option value="<?php echo $reclamos[0]?>"><?php echo $reclamos[1] ?></option> <?php } ?>        
